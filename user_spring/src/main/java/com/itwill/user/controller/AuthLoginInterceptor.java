...
public class AuthLoginInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		[ 핸들러 메소드(HandlerMethod)란? ]
			HandlerMethod는 @RequestMapping이 붙은 메소드의 정보를 추상화한 객체(reflection 객체)이다. 
			HandlerMethod는 그 자체가 실행가능한 객체(=controller)가 아니라 메소드를 실행하기 위해 필요한 
			참조정보를 담고 있는 객체 로써 다음과 같은 정보들을 가지고 있다.
				- @Controller가 붙은 컨트롤러 빈정보
			...
			디스패처 서블릿은 애플리케이션이 실행될 때 모든 컨트롤러 빈의 메소드를 살펴서(reflection) 
